...
class PlayAtPracticeSimulatedController extends ChangeNotifier {
  double scale = 1.0;
  double previousScale = 1.0;
  bool inAsyncValue = false;
  bool isDragging = false;
  List<bool> pairLength = [true,false,false];
  int currentIndexValue = 1;
  int currentHoleIndexValue = 1;
  List<Offset> points = [
    const Offset(280, 500),Offset(230, 300),Offset(150, 120),
  ];
    ...
    if (!isDragging) {
      if (pairLength[currentIndexValue]) {
       ...
       // pairLength.add(false);
        pairLength.insert(currentIndexValue, false);
        //points.add(details.localPosition);
        points.insert(currentIndexValue, details.localPosition);
        notifyListeners();
        if (calculateDistance(details.localPosition, Offset(150, 120)) < 20) {
  ...
  void onLongPress({required int index}) {
    if(points.length-1==index || index==0){
      print('You can not remove hole.......');
    }else{
